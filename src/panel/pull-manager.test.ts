...
function vaultObj(local: string) {
  ...
  const vault: VaultOps = { exists: async () => true, createFolder: async () => {}, createStub: async () => {}, writeText: async (p, d) => { writes.push({ path: p, content: d }); }, writeBinary: async () => {}, readText: async () => local, readBinary: async () => new ArrayBuffer(0), remove: async () => {}, isEmptyFolder: () => false };
  return { vault, writes };
}
